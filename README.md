...
### 4a) Alternative - Reference-Guided Assembly
If there is no sequencing of a RIL/DH population available, the contigs can be grouped by performing a reference-guided assembly. 

Necessary software: RagTag

Run RagTag scaffold to order and orient the polished contigs by mapping to the reference assembly. For best results, the reference assembly should be of the same species. The minimap2 -x parameter can be altered based on the draft assembly's divergence to the reference. 

Sample:
```
ragtag.py scaffold --aligner minimap2 --mm2-params '-x asm20' ref.fasta assembly.cleaned.fasta
```

Use the groups generated by the scaffolding to create the clusters.txt file used in step 5.

Sample:
```
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr1 | cut -f 6 | tr '\n' ' ' > group1
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr2 | cut -f 6 | tr '\n' ' ' > group2
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr3 | cut -f 6 | tr '\n' ' ' > group3
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr4 | cut -f 6 | tr '\n' ' ' > group4
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr5 | cut -f 6 | tr '\n' ' ' > group5
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr6 | cut -f 6 | tr '\n' ' ' > group6
cat ragtag.scaffold.agp | grep -v "#" | grep -v scaffold | grep Chr7 | cut -f 6 | tr '\n' ' ' > group7
# manually edit in lg# and number of contigs
cat group* > clusters.txt
```
